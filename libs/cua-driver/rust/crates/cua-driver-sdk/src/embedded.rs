...
mod tests {
    ...
    #[test]
    fn telemetry_preferences_are_inherited_but_not_overridable() {
        assert!(inherited_runtime_environment_name(
            "CUA_DRIVER_RS_TELEMETRY_ENABLED"
        ));
        assert!(inherited_runtime_environment_name("cua_telemetry_enabled"));
        ...
        assert!(!values.iter().any(|variable| variable.value == "0"));
        assert!(!values.iter().any(|variable| variable.value == "false"));
    }
