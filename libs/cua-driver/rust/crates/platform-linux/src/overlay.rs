    ...
    match backend {
        WaylandOverlayBackend::SemanticShellHelper | WaylandOverlayBackend::LegacyShellHelper => {
            ...
            match msg {
                ...
                OverlayMsg::Remove(key) => crate::wayland::shell_helper::hide_cursor(key),
                OverlayMsg::Revive(_) => {}
            }
        }
...
fn dispatch_shell_helper_command(key: &str, cmd: &OverlayCommand, semantic: bool) {
    if semantic {
        crate::wayland::shell_helper::set_cursor_color(key, &cursor_overlay::session_fill_hex(key));
    }
    match cmd {
        OverlayCommand::ClickPulse { x, y } if semantic => {
            crate::wayland::shell_helper::click_pulse(key, *x as i32, *y as i32);
        }
        OverlayCommand::MoveTo { x, y, .. }
        | OverlayCommand::SnapTo { x, y, .. }
        | OverlayCommand::ClickPulse { x, y } => {
            crate::wayland::shell_helper::move_cursor(key, *x as i32, *y as i32);
        }
        OverlayCommand::BeginAction {
            action,
            delivery,
        ...
        OverlayCommand::EndAction(action) if semantic => {
            crate::wayland::shell_helper::set_cursor_state(key, action.as_str(), "", "", false);
        }
        ...
        OverlayCommand::SetEnabled(false) => crate::wayland::shell_helper::hide_cursor(key),
        _ => {}
    }
}
