...
| KDE/KWin | No immutable local-install payload currently carries the ABI-specific helper, so production readiness and helper-backed exact identity fail closed. Parser/correlation contracts remain covered for future packaging work. | Raw target-addressed keyboard and pointer actions are intentionally refused. Numeric effect tokens can recycle across reloads and are not mutation proof. KWin support remains a gap until the immutable install closure carries a compatible helper with non-recyclable identity and mutation is bound to that identity. |
