...
//! `LegacyElementCacheCore<CacheKey, CachedSnapshot>`. Specialised
...
pub struct LegacyElementCacheCore<K: Eq + Hash, S> {
    inner: Mutex<HashMap<K, S>>,
}

impl<K: Eq + Hash, S> LegacyElementCacheCore<K, S> {
    pub fn new() -> Self {
        Self {
...
impl<K: Eq + Hash, S> Default for LegacyElementCacheCore<K, S> {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn insert_then_get_returns_projection() {
        let cache: LegacyElementCacheCore<TestKey, TestSnapshot> = LegacyElementCacheCore::new();
        let key = TestKey {
            pid: 42,
            window_id: 7,
        };
    ...
    fn insert_replaces_existing_and_runs_drop() {
        ...
        let cache: LegacyElementCacheCore<u32, DropCounter> = LegacyElementCacheCore::new();
    ...
    fn default_impl_matches_new() {
        let _cache: LegacyElementCacheCore<TestKey, TestSnapshot> =
            LegacyElementCacheCore::default();
    }
}
