...
mod tests {
    ...
    fn resolve(
        ...
        snapshot: u32,
        index: usize,
    ) -> Option<cua_driver_core::element_cache::AdmittedElement<RetainedElement>> {
        match cache
    ...
    fn retained_element_survives_concurrent_snapshot_replace() {
        ...
        let snapshot = cache.publish(std::process::id() as i32, 2, payload(ptr));
        ...
        let candidate = registry
            .prepare_current(
                ...
                2,
                0,
                std::sync::Arc::new(CachedSnapshot::from_nodes(&[])),
            )
            .unwrap();
        assert_eq!(
            registry.publish(candidate, std::time::Duration::ZERO),
            Err(cua_driver_core::element_token::RegistryError::PublicationTimeout)
        );
        drop(guard);
        cache.publish(
            std::process::id() as i32,
    ...
    fn abandoned_preparation_releases_native_payload_without_replacing_snapshot() {
        ...
        let cache = ElementCache::new();
        let snapshot = cache.publish(std::process::id() as i32, 2, payload(original_ptr));
