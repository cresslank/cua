...
fn create_private_worker_for_client(
    ...
    let (startup_timeout, startup_deadline) =
        worker::private_worker_startup_timing(options.startup_timeout_ms)?;
    ...
    Ok(Arc::new(CuaDriver {
        backend: DriverBackend::PrivateWorker(PrivateWorkerClient::spawn(
            validated,
            startup_deadline,
        )?),
        client_kind,
...
mod tests {
    ...
    fn desktop_free_compatibility_driver() -> Arc<CuaDriver> {
    ...
    fn desktop_free_configured_driver(options: ConfiguredDriverOptions) -> Arc<CuaDriver> {
        let native = NativeAbiDriver::create_configured_for_host(
            configured_driver_options_json(&options),
        ...
        .unwrap();
        Arc::new(CuaDriver {
            backend: DriverBackend::Embedded(Arc::new(native)),
            client_kind: DaemonClientKind::Unknown,
        })
    }
    ...
    fn configured_standard_driver() -> Arc<CuaDriver> {
        desktop_free_configured_driver(ConfiguredDriverOptions {
            claude_code_compatibility: false,
    ...
    async fn host_tool_inspection_does_not_acquire_runtime_ownership() {
        ...
        let driver = desktop_free_compatibility_driver();
    ...
    async fn direct_runtimes_have_independent_sessions_and_shutdown() {
        ...
        let second = desktop_free_configured_driver(ConfiguredDriverOptions {
            claude_code_compatibility: false,
            authorization: RuntimeAuthorizationOptions {
                allowed_modes: vec![
    ...
    async fn authorization_expiry_in_one_runtime_does_not_affect_another() {
        let _runtime_test = crate::runtime::TEST_RUNTIME_LOCK.lock().unwrap();
        let expiring = desktop_free_configured_driver(ConfiguredDriverOptions {
            claude_code_compatibility: false,
            authorization: RuntimeAuthorizationOptions {
                allowed_modes: vec![SessionPermissionMode::Standard],
                compatibility_mode: SessionPermissionMode::Standard,
    ...
    async fn trusted_session_is_bound_in_memory_and_rejects_public_id_substitution() {
        let _runtime_test = crate::runtime::TEST_RUNTIME_LOCK.lock().unwrap();
        let driver = desktop_free_configured_driver(ConfiguredDriverOptions {
            claude_code_compatibility: false,
            authorization: RuntimeAuthorizationOptions {
                allowed_modes: vec![
                    SessionPermissionMode::Standard,
