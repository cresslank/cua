...
//!   (or XPutImage fallback) with XRender ARGB compositing while cursors are
//!   active, then parks on the command channel when every cursor is quiescent.
...
//! - Z-ordering is reasserted while rendering so the overlay stays above the
//!   active window without burning CPU while idle.
