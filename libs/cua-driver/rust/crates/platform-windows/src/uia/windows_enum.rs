...
pub(crate) fn try_invoke_in_window_at_point(
    hwnd: isize,
    expected_pid: u32,
    sx: i32,
    sy: i32,
    ...
    PointInvokeOutcome::from_worker(run_uia_with_deadline_cancelable(
        ...
        move |cancelled| {
            try_invoke_in_window_at_point_unbounded(hwnd, expected_pid, sx, sy, &cancelled)
        },
    ))
}
...
fn try_invoke_in_window_at_point_unbounded(
    ...
    unsafe {
        ...
        // Gate foreground-steal suppression on the exact host PID, then prove
        // the winning element still belongs to that same target immediately
        // before invoking its pattern.
        crate::uia::fg_bypass::run_with_uwp_bypass_for_pid(
            hwnd,
            expected_pid,
            || -> anyhow::Result<PointInvokeOutcome> {
            if cancelled.load(Ordering::Acquire) {
                return Ok(Timeout);
            }
            if winner_has_expand {
                ...
                if let Ok(pat) = winner.GetCurrentPattern(
                        ...
                        .cast::<windows::Win32::UI::Accessibility::IUIAutomationExpandCollapsePattern>()
                    {
                        ...
                        if crate::uia::prove_element_mutation_target(
                            &winner,
