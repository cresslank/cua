...
On Linux that signal lives under the session accessibility status. On known non-Cinnamon desktops Cua enables the generic `IsEnabled` status by default, which warms up GTK and Qt without claiming that a screen reader is active; Cinnamon and unknown desktop identities leave the session status untouched. Recognized Chromium and Electron direct commands, common wrappers, and resolved URL-handler launches receive `--force-renderer-accessibility` so their renderer tree is enabled per process rather than by changing the whole desktop session. An unresolved `xdg-open` fallback cannot receive process-specific arguments. Already-running Chromium-family apps may still need the explicit compatibility mode `CUA_DRIVER_RS_A11Y_ADVERTISE_MODE=all` or a browser DevTools route when a full AT-SPI tree is required. You still need AT-SPI reachable on the session bus, and on GNOME you may also need:
