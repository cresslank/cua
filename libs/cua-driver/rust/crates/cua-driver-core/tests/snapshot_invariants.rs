    ...
    cache
        ...
        .map(|result| match result {
            ResolvedElement::Element {
                ...
                element,
                ..
            } => (window, element_index, (*element).clone()),
...
fn empty_snapshot_has_no_resolvable_members() {
    ...
    let snapshot = cache.publish(std::process::id() as i32, 7, payload(Vec::<usize>::new()));
    assert!(
...
fn replacement_invalidates_every_old_member_and_admits_new_members() {
    let cache = ElementCacheCore::new();
    let first = cache.publish(std::process::id() as i32, 7, payload(vec![0, 1]));
    let second = cache.publish(std::process::id() as i32, 7, payload(vec![10, 11]));
...
fn resolving_does_not_change_publication_order_eviction() {
    let cache = ElementCacheCore::new();
    let first = cache.publish(std::process::id() as i32, 1, payload(vec![1]));
    for window in 2..=LRU_CAP_PER_PID as u64 {
        cache.publish(std::process::id() as i32, window, payload(vec![1]));
    }
    assert_eq!(resolve(&cache, first, 0), Ok((1, 0, 1)));
    let latest = cache.publish(
        std::process::id() as i32,
        LRU_CAP_PER_PID as u64 + 1,
        payload(vec![1]),
    );
...
fn clearing_one_runtime_preserves_other_runtime_same_window() {
    let make = |scope: &str| {
        with_runtime_scope(scope.into(), || {
            ...
            let id = cache.publish(std::process::id() as i32, 7, payload(vec![1]));
            (cache, id)
        })
    };
