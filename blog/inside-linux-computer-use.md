...
On Linux that signal lives under the session accessibility status. Cua enables the generic `IsEnabled` status by default, which warms up GTK and Qt without claiming that a screen reader is active. Cua-launched Chromium and Electron processes receive `--force-renderer-accessibility` so their renderer tree is enabled per process rather than by changing the whole desktop session. Already-running Chromium-family apps may still need the explicit compatibility mode `CUA_DRIVER_RS_A11Y_ADVERTISE_MODE=all` or a browser DevTools route when a full AT-SPI tree is required. You still need AT-SPI reachable on the session bus, and on GNOME you may also need:
