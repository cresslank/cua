...
const BROWSER_CLEANUP_TIMEOUT: Duration = Duration::from_secs(20);

fn run_existing_profile_cleanup<T: Send + 'static>(
    cleanup: impl FnOnce() -> T + Send + 'static,
) -> Result<T, BrowserRefusal> {
    cua_driver_core::blocking::run_browser_cleanup(BROWSER_CLEANUP_TIMEOUT, cleanup).map_err(
        |error| {
            refusal(
                BrowserRefusalCode::BrowserRouteUnavailable,
                format!("exact browser cleanup could not complete: {error}"),
            )
        },
    )
}
...
fn browser_command_line_blocking(pid: u32) -> Result<String, BrowserRefusal> {
    let powershell = system_powershell_path()?;
    let script = format!(
        "$OutputEncoding=[Console]::OutputEncoding=[Text.UTF8Encoding]::new($false); (Get-CimInstance Win32_Process -Filter 'ProcessId = {pid}' -ErrorAction Stop).CommandLine"
    );
...
async fn exact_profile_path(
    pid: u32,
    product: BrowserProduct,
) -> Result<Option<PathBuf>, BrowserRefusal> {
    if existing_profile_setup_descriptor(product).is_none() {
        return Ok(None);
    }
    let command_line = browser_command_line(pid).await?;
    user_data_dir_from_command_line(&command_line, default_user_data_dir(product))
}

fn exact_profile_path_blocking(
    pid: u32,
    product: BrowserProduct,
) -> Result<Option<PathBuf>, BrowserRefusal> {
    if existing_profile_setup_descriptor(product).is_none() {
    ...
    let command_line = browser_command_line_blocking(pid)?;
    user_data_dir_from_command_line(&command_line, default_user_data_dir(product))
}

fn ensure_profile_discoverable(
...
async fn raw_loopback_listeners_for_process_tree(
    ...
    let allowed_pids =
        cua_driver_core::blocking::spawn(move || crate::win32::list_descendants(root_pid))
            .await
            .map_err(|error| {
                refusal(
                    BrowserRefusalCode::BrowserRouteUnavailable,
...
async fn loopback_listeners_for_process_tree(
    ...
    let tree = cua_driver_core::blocking::spawn(move || {
...
async fn loopback_listeners_for_exact_pid(pid: u32) -> Result<Vec<(u16, u32)>, BrowserRefusal> {
    let expected_started =
        cua_driver_core::blocking::spawn(move || process_identity(pid).map(|identity| identity.0))
            .await
            .map_err(|error| {
                refusal(
                    BrowserRefusalCode::BrowserRouteUnavailable,
...
async fn embedded_browser_endpoints_once(
    ...
    for (port, listener_pid) in loopback_listeners_for_process_tree(pid).await? {
        let is_webview_runtime = cua_driver_core::blocking::spawn(move || {
...
impl BrowserPlatform for WindowsBrowserPlatform {
    ...
    async fn classify_browser(&self, pid: i64) -> Result<BrowserClassification, BrowserRefusal> {
        ...
        let name = cua_driver_core::blocking::spawn(move || {
    ...
    async fn discover_owned_endpoint(
        ...
        let profile_path = exact_profile_path(pid_u32, classification.product_kind).await?;
        ensure_profile_discoverable(profile_path.as_deref())?;
        let endpoints = browser_endpoints_for_pid(pid_u32).await?;
        ensure_profile_discoverable(profile_path.as_deref())?;
    ...
    async fn discover_existing_profile_endpoint(
        ...
        if let Some(profile_path) = profile_path.as_deref() {
            if let Some(endpoint) = active_port_endpoint(pid_u32, profile_path).await? {
                ensure_profile_discoverable(Some(profile_path))?;
                return Ok(Some(endpoint));
            }
        }
        let endpoints = exact_browser_endpoints_for_pid(pid_u32).await?;
        ensure_profile_discoverable(profile_path.as_deref())?;
        select_unique_owned_endpoint(
            pid,
            endpoints,
    ...
    async fn setup_existing_profile_endpoint(
        ...
        let profile_path = exact_profile_path(pid_u32, request.browser)
            .await?
            .ok_or_else(|| {
                refusal(
                    BrowserRefusalCode::BrowserBindingStale,
        ...
        let setup_profile_path = profile_path.clone();
        let handle = cua_driver_core::blocking::spawn(move || {
            crate::browser_setup_ui::enable(hwnd, pid_u32, descriptor, &setup_profile_path)
        })
        .await
        .map_err(|error| {
            refusal(
        ...
        let endpoint_result = loop {
            match active_port_endpoint(pid_u32, &profile_path).await {
    ...
    fn cleanup_existing_profile_setup(
        ...
        let profile_path = exact_profile_path_blocking(pid, request.browser)?.ok_or_else(|| {
            refusal(
                BrowserRefusalCode::BrowserBindingStale,
                "could not identify the approved browser instance/profile resource for cleanup",
            )
        })?;
        ...
        run_existing_profile_cleanup(move || {
            ...
            let closed_setup_page =
                crate::browser_setup_ui::disable(window_id, pid, descriptor, &profile_path)?;
    ...
    async fn process_fingerprint(&self, pid: i64) -> Result<ProcessFingerprint, BrowserRefusal> {
        ...
        let (start_time, executable) =
            cua_driver_core::blocking::spawn(move || process_identity(pid_u32))
                .await
                .map_err(|error| {
                    refusal(
                        BrowserRefusalCode::BrowserRouteUnavailable,
