...
impl SetupReservation {
    fn acquire(key: PendingSetupKey) -> Result<Self, BrowserRefusal> {
        if !setup_reservations().lock().unwrap().insert(key) {
            return Err(refusal(
...
impl Drop for SetupReservation {
    fn drop(&mut self) {
        if self.release_on_drop {
            setup_reservations().lock().unwrap().remove(&self.key);
        }
    }
}
...
impl Drop for SetupUiHandle {
    fn drop(&mut self) {
        ...
        let restored = self.rollback_remote_debugging(self.pid, self.window_id);
        let closed = if let Some(element) = self.close_button.take() {
            let closed =
                unsafe { perform_action(element as AXUIElementRef, "AXPress") } == kAXErrorSuccess;
            unsafe { CFRelease(element as CFTypeRef) };
            closed
        } else {
            true
...
fn setup_reservations() -> &'static Mutex<HashSet<PendingSetupKey>> {
    static RESERVATIONS: OnceLock<Mutex<HashSet<PendingSetupKey>>> = OnceLock::new();
    RESERVATIONS.get_or_init(|| Mutex::new(HashSet::new()))
}

pub fn ensure_profile_discoverable(pid: i32) -> Result<(), BrowserRefusal> {
    if setup_reservations().lock().unwrap().contains(&pid) {
        return Err(refusal(
            BrowserRefusalCode::BrowserBindingAmbiguous,
            "browser endpoint discovery is blocked by a pending or poisoned setup for this profile",
        ));
    }
    Ok(())
}
...
pub fn retain_pending(
    ...
    if handle.reservation.key != pid || handle.pid != pid || handle.window_id != window_id {
...
pub fn abort_pending(pid: i32, window_id: u32, error: BrowserRefusal) -> BrowserRefusal {
    match pending_setups().lock().unwrap().remove(&pid) {
        Some(handle) if handle.window_id == window_id => handle.abort(pid, window_id, error),
...
pub fn enable(
    ...
    let mut handle = match initial_checkbox {
        ...
        Ok(None) => {
            ...
            // A tab mutation has occurred. Any early/unknown cleanup path must
            // keep the profile poisoned until it is proven closed.
            reservation
                .as_mut()
...
mod tests {
    ...
    #[test]
    fn concurrent_setup_and_discovery_are_serialized_per_browser_profile() {
        let pid = i32::MAX;
        let first = SetupReservation::acquire(pid).unwrap();
        assert!(SetupReservation::acquire(pid).is_err());
        assert!(ensure_profile_discoverable(pid).is_err());
        drop(first);
        ensure_profile_discoverable(pid).unwrap();
    }

    #[test]
    fn poisoned_setup_blocks_endpoint_discovery_fail_closed() {
        let pid = i32::MAX - 1;
        let mut reservation = SetupReservation::acquire(pid).unwrap();
        reservation.retain_fail_closed();
        drop(reservation);
        assert!(ensure_profile_discoverable(pid).is_err());
        setup_reservations().lock().unwrap().remove(&pid);
        ensure_profile_discoverable(pid).unwrap();
    }
