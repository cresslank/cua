...
/// Supersede element authority for a capture-only observation.
///
/// A zero-element snapshot is an unaddressable tombstone: publishing it uses
/// the registry's existing generation/lifecycle barrier to invalidate every
/// prior token for this exact lane, but no element index can resolve against
/// it and its private handle is never returned. Clear the AX cache only after
/// the tombstone is current so a successful capture-only response can never
/// leave an old token/cache pair actionable.
fn invalidate_capture_only_authority(
...
mod window_scope_contract_tests {
    ...
    #[test]
    fn capture_only_snapshot_rejects_prior_token_and_clears_ax_cache() {
        let pid = std::process::id() as i32;
        let window_id = 0xca77_0001;
        let registry = cua_driver_core::element_token::global();
        let old_snapshot = registry.register_snapshot(pid, window_id, 1).unwrap();
        let old_token = cua_driver_core::element_token::token_for(old_snapshot, 0);
        assert_eq!(
            registry.resolve(pid, &old_token).unwrap(),
            (u64::from(window_id), 0)
        );

        let cache = crate::ax::cache::ElementCache::new();
        let cached = [super::tests::node(
            Some(0),
            "AXButton",
            Some("Old"),
            0,
