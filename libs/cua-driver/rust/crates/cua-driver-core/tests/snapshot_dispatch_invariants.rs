...
// Use a real, owned process incarnation, never the authorization host itself.
struct TargetProcess(Mutex<std::process::Child>);
impl Drop for TargetProcess {
    fn drop(&mut self) {
        let child = self.0.get_mut().unwrap();
        let _ = child.kill();
        let _ = child.wait();
    }
}
fn target_slot() -> &'static Mutex<std::sync::Weak<TargetProcess>> {
    static SLOT: std::sync::OnceLock<Mutex<std::sync::Weak<TargetProcess>>> =
        std::sync::OnceLock::new();
    SLOT.get_or_init(|| Mutex::new(std::sync::Weak::new()))
}
fn target_process() -> Arc<TargetProcess> {
    let mut slot = target_slot().lock().unwrap();
    if let Some(target) = slot.upgrade() {
        return target;
    }
    let target = Arc::new(TargetProcess(Mutex::new(
...
fn pid() -> i32 {
    let target = target_slot().lock().unwrap().upgrade().unwrap();
    let pid = target.0.lock().unwrap().id();
    pid as i32
}
const WINDOW: u64 = 17;
